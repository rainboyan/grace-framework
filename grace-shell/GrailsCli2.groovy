...
class GrailsCli {
    ...
    private void initializeProfile() {
        ...
        if (new File(BuildSettings.BASE_DIR, 'profile.yml').exists()) {
            ...
            // Build Grace classpath and set system property
            List<URL> allUrls = new ArrayList<>(urls)
            List<String> allPaths = allUrls.unique().collect { URL url -> url.file }
            String[] classpath = StringUtils.tokenizeToStringArray(System.getProperty("java.class.path"), File.pathSeparator)
            // This is a workaround for allPaths.addAll(classpath)
            // Don't add 'slf4j-simple' because 'logback-classic' already exist in the classpath
            for (String cp : classpath) {
                if (!cp.contains('slf4j-simple')) {
                    allPaths.add(cp)
                }
            }

            if (BuildSettings.CLASSES_DIR.canRead()) {
                allPaths.add(BuildSettings.CLASSES_DIR.getCanonicalPath())
            }
            if (BuildSettings.RESOURCES_DIR.canRead()) {
                allPaths.add(BuildSettings.RESOURCES_DIR.getCanonicalPath())
            }

            String graceClassPath = StringUtils.collectionToDelimitedString(allPaths.unique(), File.pathSeparator)
            System.setProperty("grace.class.path", graceClassPath)
            println "GrailsCLI, grace.class.path=" + System.getProperty("grace.class.path")
            println System.getProperty("grace.class.path")
        }
        else {
            this.profileRepository = createMavenProfileRepository()
            System.setProperty("grace.class.path", System.getProperty("java.class.path"))
            println "GrailsCLI, java.class.path=" + System.getProperty("java.class.path")
